...
L'amorçage avec PXE (Pre-boot eXecution Environment) permet à un ordinateur, une station de travail, une appliance de démarrer depuis le réseau en récupérant une image d'un système d'exploitation qui se trouve sur un serveur.

Cet amorçage s'effectue généralement en 3 étapes:

1. recherche depuis une interface réseau d'une IP depuis un serveur DHCP ou BOOTP, ainsi que le fichier à amorcer (dans notre cas, les fichiers d'amorçage proviennent de [Netboot.xyz](https://netboot.xyz/))
2. téléchargement du fichier à amorcer depuis un serveur TFTP
3. exécution du fichier à amorcer

La taille du fichier à amorcer ne permet pas de démarrer directement un noyau Linux (alors qu'on peut avec des BSD car les noyaux sont moins gros), mais il faut que le logiciel à amorcer le télécharge et l'exécute lui-même.

C'est exactement le rôle de [Netboot.xyz](https://netboot.xyz/): amorcer puis télécharger l'OS à démarrer et installer.

On peut aussi faire des séquences de démarrage un peu plus compliquée comme par exemple avec NetBSD: on peut utiliser PXELINUX pour charger le netboot loader de NetBSD qui se chargera ensuite de démarrer ensuite le noyau NetBSD.
...
Chez moi j'utilise OPNSense depuis bien longtemps (avant ça PFSense et encore avant m0n0wall), on va donc voir comment le configurer pour pourvoir amorcer un OS depuis le réseau.

En effet OPNSense me sert déjà pour mon routage, gérer mes machines en DHCP ainsi que le DNS avec unbound DNS.
...
A noter que bien que j'ai téléchargé le fichier netboot.xyz-arm64.efi pour les architectures arm64, je ne l'utiliserai pas je n'ai pas d'arm pour démarrer dessus.

Il nous reste à démarrer le serveur TFTP.
- on active le service TFTP en cliquant sur "**Enable**"
- on indique l'IP depuis laquelle le daemon TFTP écoutera (j'aurais pu éviter 0.0.0.0!)
- on clique sur le bouton "**Save**"
- le service devrait démarrer

![opnsense-tftp-enable](images/opnsense-tftp-enable.png)

On peut aller vérifier en allant dans le menu "**System > Diagnostics > Services**" que le service est bien démarré.

![opnsense-tftp-started](images/opnsense-tftp-started.png)

On peut aussi vérifier depuis client tftp sur une autre machine étant donné que le daemon tftp écoute sur 0.0.0.0:
...
Tout est ok, on peut maintenant configurer notre serveur DHCP.

### Configuration du serveur DHCP

La configuration du serveur DHCP dans OPNsense n'est pas très compliquée. Par contre ici on part du principe que vous avez déjà un serveur DHCP configuré et activé.

- aller dans le menu "**Services > ISC DHCPv4**"
- sélectionner l'interface que vous utilisez, ici c'est "**LAN**" qui correspond à mon réseau interne(ça pourrait être OPT1, VLAN1, etc... suivant votre configuration)
- dans les différents paramètres, recherche "**Network booting**"
...
- enfin renseigner les champs comme indiqués dans la capture d'écran en dessous
  - on retrouve l'IP du prochain serveur, en fait le serveur TFTP sur lequel la machine devra se connecter pour télécharger son fichier d'amorçage
  - les différents fichiers d'amorçage
- on clique sur le bouton "**Save**" pour sauvegardere notre configuration DHPC
...
La voici: démarrer dessus avec un amorace PXE!

### Configuration du BIOS

D'abord il faut configurer la carte au niveau du BIOS pour qu'elle démarrage en priorité en PXE.
Pour entrer dans le BIOS, il faut appuyer sur la touche "S" pendant le test de la mémoire.
Et comme indiqué dans la documentation:

> E toggles PXE boot enable. Defaults to disable as the PXE module has a 60 second time-out.

Je n'avais pas vu et je n'ai pas essayé mais apparament on pouvait aussi appuyer sur la touche "N" pour démarrer en PXE sans modifier la configuration du BIOS.
...
Une fois le BIOS configuré, la magie opère: la carte Alix 2D3 démmarre sur le fichier d'amorace de [Netboot.xyz](https://netboot.xyz/).
...
On peut voir quelques informations interrant pendant la séquence de démarrage.

Tout d'abord la connexion réseau avec notre serveur DHCP et les tentatives de téléchargement des fichiers d'amorçage
...
Je ne rentre pas dans le détail de la création de la VM car ce n'est pas l'objet de cet article.
Il faut juste s'assurer dans la configuration que le démarrage de la VM se fait bien sur l'interface réseau en 1er et non pas le disque ni le CD-ROM.
...
- dans le menu "**Options**" de la VM
- sélectionner "**Boot Order**" et cliquer en haut sur le bouton "**Edit**"
- Configuration pour y accéder par le port série avec un terminal
...
Il suffit de modifier l'ordre et de choisir "**net0**" à mettre en haut de la liste pour démarrer en 1er lors du démarrage. Le nom de l'interface réseau peut éventuellement changer suivant votre configuration.
...
Il suffit de sélectionner "**xterm.js**" depuis le menu "**Console**"
...
Et voilà on est capable de démarrer à peu prêt n'importe quelle machine depuis le réseau en PXE pour peu que:
- elle dispose d'une interface et connexion au réseau
- que son BIOS permette de démarrer en PXE

J'ai voulu également tester ça avec BeagleBone Black doté d'un ARM Cortex A8, malheursement son BIOS ne permet pas de démarrer en PXE.
